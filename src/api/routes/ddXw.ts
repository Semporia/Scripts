...
export default (app: Router) => {
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const { code } = await serviceInstance.createCode({ name: req.params.name, code: req.params.code, type: ShareCodeType['ddXw'] });
  ...
  route.get('/', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const serviceInstance = Container.get(ShareCodeService);
      const { code } = await serviceInstance.getCode(ShareCodeType['ddXw']);
      return res.status(200).json({ code: 200, data: code });
    } catch (e) {
      logger.error('🔥 error: %o', e);
      return next(e);
  ...
  route.get('/count', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const serviceInstance = Container.get(ShareCodeService);
      const { count } = await serviceInstance.countCode(ShareCodeType['ddXw']);
  ...
  route.get('/remove', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const serviceInstance = Container.get(ShareCodeService);
      const { msg } = await serviceInstance.removeCode(ShareCodeType['ddXw']);
