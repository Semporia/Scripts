...
export default (app: Router) => {
  ...
  route.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        const serviceInstance = Container.get(ShareCodeService);
        const { code } = await serviceInstance.getCode(ShareCodeType['jdZz']);
        return res.status(200).json({ code: 200, data: code });
      } catch (e) {
        logger.error('🔥 error: %o', e);
        return next(e);
  ...
  route.get('/count', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const serviceInstance = Container.get(ShareCodeService);
      const { count } = await serviceInstance.countCode(ShareCodeType['jdZz']);
  ...
  route.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        const serviceInstance = Container.get(ShareCodeService);
        const { msg } = await serviceInstance.removeCode(ShareCodeType['jdZz']);
