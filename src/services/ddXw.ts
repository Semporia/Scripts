...
@Service()
export default class DdXwService {
  constructor(@Inject('ddXw') private ddXw: Models.ICommonModel, @Inject('logger') private logger) {}
  ...
  public async createCode({ name, code }: { name: string; code: string }): Promise<{ code: ICommon & Document }> {
    const record = await this.ddXw.findOneAndUpdate(
      { $or: [{ name, value: code }] },
      {
        value: code,
        name: name,
  ...
  public async countCode(): Promise<{ count: Number }> {
    const count = await this.ddXw.count({});
    return { count };
  }

  public async removeCode(): Promise<{ msg: string }> {
    const count = await this.ddXw.remove({});
    return { msg: 'success' };
  }
}
