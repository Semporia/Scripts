...
!(async () => {
    ...
    authorCodeList = ['7e77f321877449d4b0cc5c3d47494915']
    console.log(`入口：\nhttps://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity/5089773?activityId=${$.activityId}&shareUuid=${authorCodeList[0]}`)
...
async function member() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.index==1) $.authorCode = '7e77f321877449d4b0cc5c3d47494915';
            ...
            if ($.index === 1) {
                await task('linkgame/activity/content', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${$.attrTouXiang}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1)
            } else {
                await task('linkgame/activity/content', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${$.attrTouXiang}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`)
            }
            $.log("关注店铺")
                        ...
                        if (data.result) {
                            switch (function_id) {
                                ...
                                case 'linkgame/help/list':
                                    $.log(`已邀请: ${data.data.shareList.length}`)
                                    break;
                                default:
                                    // $.log(JSON.stringify(data))
                                    break;
