...
function doTask({ tasklevel, left, taskname }) {
  return new Promise(async resolve => {
    ...
    $.get(taskUrl('dotask', `active=${$.info.active}&joinnum=${$.info.joinnum}&tasklevel=${tasklevel}`), (err, resp, data) => {
      try {
        const res = data.match(/try\{whyour\(([\s\S]*)\)\;\}catch\(e\)\{\}/)[1];
        $.log(res)
  ...
  return new Promise(resolve => {
    $.get({ url: 'https://api.ninesix.cc/api/jx-nc' }, (err, resp, _data) => {
      try {
        const { data = {} } = JSON.parse(_data);
        $.log(`\n${data.value}\n${$.showLog ? _data : ''}`);
        $.get(taskUrl('help', `active=${$.info.active}&joinnum=${$.info.joinnum}&smp=b47f4790d7b2a024e75279f55f6249b9`), (err, resp, data) => {
          try {
            const res = data.match(/try\{whyour\(([\s\S]*)\)\;\}catch\(e\)\{\}/)[1];
            const { ret, retmsg = '' } = JSON.parse(res);
            $.log(`\n助力：${retmsg} \n${$.showLog ? res : ''}`);
