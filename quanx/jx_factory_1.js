...
!(async () => {
  ...
  for (let i = 0; i < $.cookieArr.length; i++) {
    ...
    if (currentCookie) {
      ...
      const beginInfo = await getUserInfo(currentCookie);
      await $.wait(500);
      await getCommodityDetail(currentCookie);
      await $.wait(500);
      await getCurrentElectricity(currentCookie);
      await $.wait(500);
      await getTaskList(currentCookie);
      await $.wait(500);
      await browserTask(currentCookie);
      await $.wait(500);
      await getHireRewardList(currentCookie);
      await $.wait(500);
      await stealFriend(currentCookie);
      await $.wait(500);
      await submitInviteId(userName);
      await $.wait(500);
      await createAssistUser(currentCookie);
      const endInfo = await getUserInfo(currentCookie);
      await $.wait(500);
      $.result.push(
      ...
      await investElectric(currentCookie);
      await getTuanId(currentCookie);
      await submitTuanId(userName);
      await joinTuan(currentCookie);
    }
  }
  await showMsg();
})()
...
function getUserInfo(currentCookie) {
  return new Promise((resolve) => {
    $.get(taskUrl("userinfo/GetUserInfo", {}, currentCookie), async (err, resp, data) => {
      try {
        const {
          ret,
...
function getCommodityDetail(currentCookie) {
...
function getCurrentElectricity(currentCookie) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
      ...
      async (err, resp, data) => {
        try {
          ...
          if (
            ...
            await collectElectricity($.info.factoryInfo.factoryId, '', currentCookie);
          }
        } catch (e) {
          $.logErr(e, resp);
        } finally {
...
function collectElectricity(facId, master, currentCookie) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
...
function getTaskList(currentCookie) {
  return new Promise(async (resolve) => {
    $.get(taskListUrl("GetUserTaskStatusList", {}, currentCookie), async (err, resp, data) => {
      try {
...
function browserTask(currentCookie) {
  return new Promise(async (resolve) => {
    ...
    for (let i = 0; i < $.allTask.length; i++) {
      ...
      for (let i = 0; i < times; i++) {
        await $.wait(500);
        if (status[0]) {
          status[0] = await doTask(task,currentCookie);
        }
        await $.wait(500);
        if (status[1]) {
          status[1] = await awardTask(task,currentCookie);
        }
...
function awardTask({ taskId, taskName }, currentCookie) {
  return new Promise((resolve) => {
    $.get(taskListUrl("Award", `taskId=${taskId}`, currentCookie), (err, resp, data) => {
      try {
...
function doTask({ taskId, completedTimes, configTargetTimes, taskName }, currentCookie) {
  return new Promise(async (resolve) => {
    ...
    $.get(taskListUrl("DoTask", `taskId=${taskId}`, currentCookie), (err, resp, data) => {
      try {
        const { msg, ret } = JSON.parse(data);
...
function investElectric(currentCookie) {
  return new Promise(async (resolve) => {
    if (!$.autoCharge) {
      $.result.push('未打开自动投入')
      resolve();
...
function getHireRewardList(currentCookie) {
  return new Promise(async (resolve) => {
    $.get(taskUrl("friend/QueryHireReward", `_time=${new Date().getTime()}`, currentCookie), async (err, resp, data) => {
      try {
        ...
        if (hireReward && hireReward.length > 0) {
          for (let i = 0; i < hireReward.length; i++) {
            ...
            await hireAward(`date=${date}`, currentCookie);
          }
        }
      } catch (e) {
        $.logErr(e, resp);
...
function hireAward(body, currentCookie) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl("friend/HireAward", `${body}&_time=${new Date().getTime()}&type=0`, currentCookie),
      async (err, resp, data) => {
        try {
          const { msg, data: { investElectric } = {} } = JSON.parse(data);
          $.log(`\n收取打工电力：${msg}\n${$.showLog ? data : ''}`);
...
function stealFriend(currentCookie) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl("friend/QueryFactoryManagerList", {}, currentCookie),
      async (err, resp, data) => {
        try {
          ...
          for (let i = 0; i < canCollectFriends.length; i++) {
            ...
            if (facId) {
              await collectElectricity(facId, key, currentCookie);
            }
          }
        } catch (e) {
          $.logErr(e, resp);
...
function createAssistUser(currentCookie) {
  return new Promise((resolve) => {
    $.get({ url: "https://api.ninesix.cc/jx-factory" }, (err, resp, _data) => {
      try {
        ...
        $.log(`\n${data.value}\n${$.showLog ? _data : ''}`);
        $.get(
          taskUrl("friend/AssistFriend", `sharepin=${escape(data.value)}`, currentCookie),
          async (err, resp, data) => {
            try {
...
function getTuanId(currentCookie) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl("tuan/QueryActiveConfig", `activeId=ilOin38J30PcT9xnWbx9lw%3D%3D&_time=${new Date().getTime()}`, currentCookie),
      async (err, resp, data) => {
        try {
          const { msg, data: { userTuanInfo } = {} } = JSON.parse(data);
          $.log(`\n获取团id：${msg}\n${$.showLog ? data : ''}`);
          if (!userTuanInfo.tuanId) {
            await createTuan(currentCookie);
          } else {
...
function createTuan(currentCookie) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl("tuan/CreateTuan", `activeId=ilOin38J30PcT9xnWbx9lw%3D%3D&isOpenApp=1&_time=${new Date().getTime()}`, currentCookie),
      async (err, resp, data) => {
        try {
          const { msg, data: { userTuanInfo } = {} } = JSON.parse(data);
          $.log(`\n开团信息：${msg}\n${$.showLog ? data : ''}`);
...
function joinTuan(currentCookie) {
  return new Promise(async (resolve) => {
    $.get({ url: "https://api.ninesix.cc/jx-factory-tuan" }, (err, resp, _data) => {
      try {
        ...
        $.log(`\n${data.value}\n${$.showLog ? _data : ''}`);
        $.get(
          taskUrl("tuan/JoinTuan", `activeId=ilOin38J30PcT9xnWbx9lw%3D%3D&tuanId=${escape(data.value)}&_time=${new Date().getTime()}`, currentCookie),
          async (err, resp, data) => {
            try {
              const { msg } = JSON.parse(data);
