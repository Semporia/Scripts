...
关注10京豆 (有可能是空气💨
加购5京豆 (有可能是空气💨 默认加购 如不加购请设置环境变量[qm818_addSku]为"false"

第一个账号助力作者 其他依次助力CK1
第一个CK失效会退出脚本

————————————————
入口：[8.18-8.25 全民818 一“促”即发" (https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/7768835?activityId=02d54511202b4d1781088b66b9e07b9c&shareUuid=)]
============Quantumultx===============
[task_local]
#8.18-8.25 全民818 一“促”即发"
30 9,21 18-25 8 * jd_qm818.js, tag=8.18-8.25 全民818 一“促”即发", enabled=true

================Loon==============
[Script]
cron "30 9,21 18-25 8 *" script-path=jd_qm818.js,tag=8.18-8.25 全民818 一“促”即发"

===============Surge=================
8.18-8.25 全民818 一“促”即发" = type=cron,cronexp="30 9,21 18-25 8 *",wake-system=1,timeout=3600,script-path=jd_qm818.js

============小火箭=========
8.18-8.25 全民818 一“促”即发" = type=cron,script-path=jd_qm818.js, cronexpr="30 9,21 18-25 8 *", timeout=3600, enable=true
*/
...
guaopencard_addSku = $.isNode() ? (process.env.qm818_addSku ? process.env.qm818_addSku : `${guaopencard_addSku}`) : ($.getdata('qm818_addSku') ? $.getdata('qm818_addSku') : `${guaopencard_addSku}`);
message = ""
!(async () => {
  if (!cookiesArr[0]) {
...
//  if ($.isNode()) {
//    if (!process.env.opencard_qm818 || process.env.opencard_qm818 == "false") {
//      console.log('如需执行脚本请设置环境变量[opencard_qm818]为"true"')
//      return
//    }
//  }
  $.shareUuid = ''
...
async function run(){
  try{
    ...
    $.log("加购: " + $.addSku)
    if(!$.addSku && guaopencard_addSku+"" != "true") console.log('如需加购请设置环境变量[qm818_addSku]为"true"');
