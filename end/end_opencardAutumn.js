/*
秋新资联合开卡 [jd_opencardAutumn.js]
cron 40 6,18 * * * jd_opencardAutumn.js
————————————————
...
(默认不加购 如需加购请设置环境变量[opencardAutumn_addSku]为"true"
做完浏览、关注、加购 可以领取20京豆
...
opencard_addSku = $.isNode() ? (process.env.opencardAutumn_addSku ? process.env.opencardAutumn_addSku : `${opencard_addSku}`) : ($.getdata('opencardAutumn_addSku') ? $.getdata('opencardAutumn_addSku') : `${opencard_addSku}`);
message = ""
!(async () => {
  if (!cookiesArr[0]) {
      ...
      if($.task.showOrder){
        ...
        for (let i = 0; i < $.taskList.length; i++) {
          ...
          if($.task.showOrder == 1){
            ...
            if(opencard_addSku+"" != "true" && 2 == $.oneTask.groupType) console.log('如需加购请设置环境变量[opencardAutumn_addSku]为"true"\n');
